...
export default function TasksPage() {
  ...
  useEffect(() => {
    dispatch({
      ...
      featureId: "tasks",
      data: googleTasksToAppTasks(tasks),
    });
  }, [tasks, dispatch]);
  ...
  async function handleNewTask() {
    await createTask(newTitle, newNotes, newDue);
    setNewTitle("");
    setNewNotes("");
    setNewDue("");
  ...
  async function handleAddSubtask() {
    if (!newSubtaskTitle.trim() || !addingSubtaskTo) return;
    const ok = await createSubtask(addingSubtaskTo, newSubtaskTitle);
    if (ok) {
  ...
  async function saveEditingSubtask(subtask: GoogleTask) {
    ...
    const ok = await updateTask(subtask.id, {
  ...
  async function handleAIExpand(task: GoogleTask) {
    ...
    try {
      ...
      for (const line of lines) {
        const ok = await createSubtask(task.id, line, { silent: true });
        if (!ok) throw new Error("Failed to create subtask");
      }
