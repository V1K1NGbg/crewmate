...
import { COLOR_SCHEMES, type ColorSchemeColors } from "@crewmate/types";
...
function applyColors(colors: ColorSchemeColors) {
  ...
  // Per-page identity colors: scheme override if present, else each
  // plugin's own fallback `color`. Generic over whatever plugins are
  // registered — the theme itself doesn't need to know their ids.
  for (const plugin of PLUGINS) {
    root.style.setProperty(
      `--color-${plugin.id}`,
      colors.pages?.[plugin.id] ?? plugin.color,
    );
  }
}
